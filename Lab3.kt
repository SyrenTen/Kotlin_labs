...
Завдання: Створіть функцію, яка приймає список чисел і повертає список чисел, які є добутком трьох інших чисел зі списку. */
...
fun calculateProduct(nums: List<Int>): List<Int> {
    return nums.flatMap {a ->
        nums.flatMap { b ->
            nums.mapNotNull { c ->
                if (a != b && b != c && a != c) a * b * c else null //перевірка що 3 різних числа та обчисл. добутка
            }
        }
    }
}
